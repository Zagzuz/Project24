...
public:
	...
	{
		return cells_.at((x_coord - left_coordinate()) / x_cell_size_).at((y_coord - bottom_coordinate()) / y_cell_size_);
	}
...
{
	return x_coord < right_coordinate() && x_coord > left_coordinate() && y_coord < top_coordinate() && y_coord > bottom_coordinate();
}
	...
	for (int counter = 0; std::getline(iss, token, ' '); ++counter)
	{
		...
		case 2:
				...
				creatures_it != sq.creatures_map.end())
			{
				...
				token_iss.str(token);
				token_iss >> size;
				cells_[i][j].change_squad(func(size));
			}
